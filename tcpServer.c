...
PI_THREAD(thread_aceptar_periferico) {
	...
	if (servidor.socket_fd != -1 && !servidor.aceptandoPerifericos)
		reiniciarServidor();
...
void timer_envio_pantalla_isr(union sigval value) {
	...
	if (servidor.servidorHabilitado) {
		for (int idPeriferico = 0; idPeriferico < MAX_PERIFERICOS_CONECTADOS; idPeriferico++) {
			partida = servidor.periferico[idPeriferico].partida;
...
/**
 * Renicia el socket del servidor en el puerto especificado en sistema.puerto
 * y comienza a aceptar perifericos.
 */
